...
const indianStates = useMemo(() => {
  return Object.keys(blockFloorMap).map((block) => ({
    name: block,
    isoCode: block, // just to keep same structure
  }));
}, []);

// Replace cities → floors
const cities = useMemo(() => {
  if (!selectedState) return [];
  return (blockFloorMap[selectedState] || []).map((floor) => ({
    name: floor,
  }));
}, [selectedState]);
                ...
                render={({ field }) => (
                  ...
                  >
                    <SelectTrigger className="w-full">
                      <SelectValue
                        placeholder={
                          selectedState ? "Select floor" : "Select floor first"
                        }
                      />
                    </SelectTrigger>
                    <SelectContent>
